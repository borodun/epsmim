...
//            printf("%d - Y: %d\n", thread, y);
            __m512d UUpLeft = UUp[y - 1];
            __m512d UUpCentre = UUp[y];
            __m512d UUpRight = UUp[y + 1];

            __m512d UMidLeft = UMid[y - 1];
            __m512d UMidCentre = UMid[y];
            __m512d UMidRight = UMid[y + 1];

            __m512d ULowLeft = ULow[y - 1];
            __m512d ULowCentre = ULow[y];
            __m512d ULowRight = ULow[y + 1];

            __m512d PMidLeft = PMid[y - 1];
            __m512d PMidCentre = PMid[y];
            __m512d PMidRight = PMid[y + 1];

            __m512d PLowLeft = PLow[y - 1];
            __m512d PLowCentre = PLow[y];
            __m512d PLowRight = PLow[y + 1];
            ...
            for (int x = 1; x < Nx / vectorSize; ++x) {
                ...
                __m512d res = 2 * UMidCentre - UPrevVec[x] + v_tausqr * (avgx + avgy + phi(x, y, n));
                UNextVec[x] = res;
                localVUMax = _mm512_max_pd(localVUMax, res);
...
#pragma omp critical
            {
                VUMax = _mm512_max_pd(localVUMax, VUMax);
                if (localMax > UMax) {
                    UMax = localMax;
                }
            }
...
__m512d WaveSim::vphi(int x, int y, int n) {
