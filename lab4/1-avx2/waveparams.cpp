...
WaveParams::WaveParams(int argc, char *argv[]) {
    if (argc < 8) {
        throw badArgsException(std::string("Usage: ") + argv[0] + "Nx Ny Nt Sx Sy Threads filename");
    }
    ...
    if (Threads < 1) {
        throw badArgsException("Thread count should be greater than 0");
    }
}
...
int WaveParams::getThreads() const {
    return Threads;
}
